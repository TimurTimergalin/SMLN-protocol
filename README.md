# Спецификация протокола "Secure Messaging in Local Network" (SMLN) v1.0.1
...
- Отправлять другим пользователям текстовые сообщения в беседах один на один или в многопользовательских беседах.;
...
*Закрытый ключ* используется для дешифрования пользовательских сообщений, полученных владельцем ключа. Для хранения в базе данных ключ должен быть зашифрован паролем пользователя с помощью алгоритма *scripts*. Закрытый ключ (в любом виде) является конфиденциальной информацией о пользователе.

Каждая многопользовательская беседа также содержит *идентификатор*, *открытый* и *закрытый* ключи, которые хранятся по тому же принципу (приватный ключ должен храниться в нескольких копиях - по одной на каждого пользователя беседы, зашифрованным соответствующим ключем). 
...
- 0 - успешный запрос
- 1 - ошибка клиента - неверный тип запроса
...
Тип *user* используется для представления публичной информации о пользователе. Публичная информация должна включать в себя *идентификатор пользователя* (поле `"id"`). Публичная информация пользователя может включать и другие поля (это определяется сервером).
...
В случае, если в сообщении отсутствуют необходимые поля, или значения полей имеют неправильный тип, сервер должен выдать ошибку `2`.

Никакой запрос, кроме запроса типа `"auth"`, не может быть выполнен, если до этого не былы выполнена авторизация. При попытке выполнить неавторизованный запрос, сервер должен выдать ошибку `"3"`.
...
```
{
    ...
    "args":
    {
    	...
    	"public-key": <str>,  // открытый RSA-ключ
    	"private-key": <str>  // закрытый RSA-ключ в зашифрованном виде
    }
}
```
...
Запрос типа `"messages"` возвращает массив сообщений от пользователя или из беседы.
...
- Пользователся с таким идентификатором  не существует - 2
...
Запрос типа `"download"` позволяет загрузить файл с сервера.
...
- Такого файла не существует или у пользователя нет доступа к файлу - 3
